  ...
  return (
    <>
      <div className="watch-card h-[80px] sm:h-[110px] lg:h-[100px] w-full flex flex-nowrap items-start gap-2 mb-3">
        <Link to={`/watch/${props.video.id}/`} className="w-[40%] h-full rounded-xl" style={{ background: `url('${props.video.snippet.thumbnails.default.url}') no-repeat center`, backgroundSize: "cover" }}></Link>
        <div className="details flex-1">
          <Link to={`/watch/${props.video.id}/`} className='text-sm md:text-[12px] font-semibold'>{props.video.snippet.title}</Link>
